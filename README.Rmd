...
faceplyr::install_opencv(build = "conda") # Only conda install is supported
``` 
...
```{r conda-install}
conda_install(pyV="3.8")
```

This function should return `TRUE` if all goes smoothly. If all else fails (or you're on Windows) you can install the `Docker` image with from the `Dockerfile` in this repository (TBD: `Dockerfile`).
...
- Switch to building openCV from source (for RVision) and link to conda
  - See `./R/install_opencv_source.R` for details
  - Remove requirement for `opencv-python`
  
- Switch feature saving from `.RDS` to SQLite `.db`
  - Allows for everything (sans original image) to be stored in a single database that's cross platform and program (e.g., read into python easily)
